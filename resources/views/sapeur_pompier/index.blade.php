...
<div class="row">
    <div class="col-lg-12 col-md-12">
        <button type="button" class="btn btn-primary mb-3" data-toggle="modal" data-target="#exampleModal">Ajouter sapeur pompier</button>
                ...
                @if($sapeur_pompiers->isNotEmpty() )
                    <div class="table-responsive">
                        <table class="display table table-striped table-bordered" id="language_option_table" style="width:100%">
                            ...
                            <tbody>
                                @foreach($sapeur_pompiers as $key => $item)
                                    <!-- Modal -->
                                    <div class="modal fade" id="id{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                ...
                                                <h5 class="modal-title" id="exampleModalLabel">Modifier un sapeur pompier</h5>
                                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">×</span>
                                                </button>
                                                </div>
                                                <div class="modal-body">
                                                    <form action="{{ route('update-sapeur_pompier', ['id' => $item->id]) }}" method="post">
                                                        @csrf
                                                        <div class="row">
                                                            <div class="col-12">
                                                            ...
                                                            <div class="col-12">
                                                                <div class="form-group">
                                                                    ...
                                                                    <div class="input-group">
                                                                        <input readonly type="text" class="form-control" name="email" value="{{ $item->email }}">
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="col-12">
                                                                    ...
                                                                    <div class="input-group">
                                                                        <select class="form-control" name="commune_id" id="">
                                                                            @foreach ($communes as $commune)
                                                                                <option value="{{ $commune->id }}" {{ $commune->id == $item->commune_id ? 'selected' : '' }}>{{ $commune->nom }}</option>
                                                                            @endforeach
                                                                        </select>
                                                                    </div>
                                                                </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a class="text-success mr-2" href="{{ route('incidents-sapeur_pompier', ['id' => $item->id]) }}">
                                                <button class="btn btn-success">Voir les incidents</button>
                                            </a>
                                            <form action="{{ route('destroy-sapeur_pompier', ['id' => $item->id]) }}" method="POST"  style="display:inline;">
                                                @csrf
                                                @method('DELETE')
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form action="{{ route('store-sapeur_pompier') }}" method="post">
            @csrf
            <div class="row">
                <div class="col-12">
                    <div class="form-group">
