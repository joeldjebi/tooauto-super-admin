...
@include('layouts.fileariane')
    ...
    <div class="row">
        ...
        <div class="card text-left">
            <div class="card-body">
                <h4 class="card-title mb-3">Les TV</h4>
                @if($tvs->isNotEmpty() )
                            ...
                            <tbody>
                                @foreach($tvs as $key => $item)
                                        ...
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form action="{{ route('update-tv', ['id' => $item->id]) }}" method="post">
                                                        @csrf
                                                        <div class="col-md-12">
                                                        ...
                                                        <div class="col-md-12">
                                                            <div class="form-group">
                                                                ...
                                                                <select class="form-control" name="categorie_tv" id="">
                                                                    @foreach ($categorie_tvs as $p)
                                                                        <option value="{{ $p->id }}" {{ $p->id == $item->categorie_tv_id ? 'selected' : ''}}>{{ $p->libelle }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                        </div>
                                                        <div class="col-md-12">
                                                            <div class="form-group">
                                                                <label for="">URL</label>
                                                                <input class="form-control" name="url" type="text" value="{{ old("url")?? $item->url }}">
                                                            </div>
                                                        </div>
                                    ...
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ $item->libelle }}</td>
                                        <td>{{ $item->categorie_tv->libelle }}</td>
                                        <td>{{ $item->url }}</td>
                                        <td>
                                            <a class="text-success mr-2" href="#" data-toggle="modal" data-target="#id{{ $item->id }}">
                                                <i class="nav-icon i-Pen-2 font-weight-bold"></i>
                                            </a>
                                            <form action="{{ route('delete-tv', ['id' => $item->id]) }}" method="POST"  style="display:inline;">
                                                @csrf
                                                @method('DELETE')
                    ...
                    @else
                        <p>Aucune TV enregistrer !</p>
                @endif
            </div>
        </div>
        </div>
        <div class="col-lg-4 col-md-12">
            <h4>Ajouter une TV</h4>
            <div class="card mb-5">
                <div class="card-body">
                    <div class="d-flex flex-column">
                        <form action="{{ route('store-tv') }}" method="post">
                            @csrf
                            <div class="form-group">
                                <label for="">Libellé</label>
