...
@include('layouts.fileariane')
    ...
    <div class="row">
        ...
        <div class="card text-left">
            <div class="card-body">
                <h4 class="card-title mb-3">Les contacts utils</h4>
                @if($contact_utils->isNotEmpty() )
                    <div class="table-responsive">
                        <table class="display table table-striped table-bordered" id="language_option_table" style="width:100%">
                            ...
                            <tbody>
                                @foreach($contact_utils as $key => $item)
                                        ...
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="exampleModalLabel">Modifier l'information</h5>
                                                    <button class="close" type="button" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                                                </div>
                                                <div class="modal-body">
                                                    <form action="{{ route('update-contact-util', ['id' => $item->id]) }}" method="post">
                                                        @csrf
                                                        <div class="col-md-12">
                                                        ...
                                                        <div class="col-md-12">
                                                            <div class="form-group">
                                                                <label for="">Fix</label>
                                                                <input class="form-control" name="fix" type="number" value="{{ old("fix")?? $item->fix }}">
                                                            </div>
                                                        </div>
                                                        <div class="col-md-12">
                                                            <div class="form-group">
                                                                <label for="">Adresse</label>
                                                                <textarea class="form-control" name="adresse" rows="3">{{ old("adresse")?? $item->adresse }}</textarea>
                                                            </div>
                                                        </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <form action="{{ route('delete-contact-util', ['id' => $item->id]) }}" method="POST"  style="display:inline;">
                                                @csrf
                                                @method('DELETE')
                    ...
                    @else
                        <p>Aucun contact util enregistrer !'</p>
                @endif
            </div>
        </div>
        </div>
        <div class="col-lg-4 col-md-12">
            <h4>Ajouter un contact util</h4>
            <div class="card mb-5">
                <div class="card-body">
                    <div class="d-flex flex-column">
                        <form action="{{ route('store-contact-util') }}" method="post">
                            @csrf
                            <div class="form-group">
                                <label for="">Libellé</label>
                            ...
                            <div class="form-group">
                                <label for="">Fix</label>
                                <input class="form-control" name="fix" type="number">
                            </div>
                            <div class="form-group">
                                <label for="">Adresse</label>
